...
SETUP_CASE=fates_ctsm_edison_fullmodel_bci_parameter_ensemble_1pft_v010_metaensemble
...
for i_case  in `seq 1 1 $ncases`; do
    casenumberstring=$(printf %02d $i_case)
    CASE_NAME=${SETUP_CASE}_enssubset${casenumberstring}_${GITHASH1}_${GITHASH2}
    echo $i_case
    if ((i_case == 1)); then
	echo "here"
	./create_newcase --case ${CASE_NAME} --res ${RES} --compset I2000Clm50FatesGs --mach edison --project $project --run-unsupported
	first_casename=${CASE_NAME}
    else
	echo "not here"
	./create_clone --case ${CASE_NAME} --clone ${first_casename} --project $project --keepexe
    fi
    ...
    ./xmlchange NINST_LND=$ninst_per_case
    ./xmlchange ROOTPE_LND=1

    for x  in `seq 1 1 $ninst_per_case`; do
	expstr=$(printf %04d $x)
	paramfilestring=$(printf %04d $(($x + ($i_case-1)*$ninst_per_case)))
	echo $expstr $paramfilestring
	cat > user_nl_clm_$expstr <<EOF
fsurdat = '${CLM_SURFDAT_DIR}/${CLM_USRDAT_SURDAT}'
fates_paramfile = '/global/homes/c/cdkoven/scratch/parameter_file_sandbox/fates_params_358720c_bcitraits_test02_v_${paramfilestring}.c180925.nc'
